...
def MCMC_experiment_poster(d, img, data, seed, candidate_angles, sigma2,
                         em_params1=dict(), em_params2=dict(), vg_params=dict(), 
                         mala_params=dict(), save_dir='MCMCexp'):
    ...
    vg_counts_all = []
    k = len(candidate_angles)
    for i in range(len(vg_samples_zs)):
        dists = np.abs(vg_samples_zs[-i][:, None] - candidate_angles[None, :])
        idx = np.argmin(dists, axis=1)
        mask = np.isclose(vg_samples_zs[-i], candidate_angles[idx], atol=1e-6)
        idx = idx[mask]
        vg_counts = np.bincount(idx, minlength=k)
        vg_counts_all.append(vg_counts.copy())

    # Compute distances ld
    mala_counts_all = []
    for i in range(len(mala_samples_zs)):
        mala_counts = np.bincount(mala_samples_zs[-i], minlength=k)
        mala_counts_all.append(mala_counts.copy())
    ...
    # Column 0: ground truth, EM
    im = axes[0][0].imshow(img, cmap='gray', vmin=vmin, vmax=vmax)   # <-- changed
    axes[0][0].set_title("Ground truth")
    ...
    axes[0][3].axis("off")
    axes[0][4].bar(list(range(k)), np.mean(vg_counts_all, axis=0), width=1.0)
    axes[0][4].set_title("VG mean assignments")
    axes[0][4].axis("off")

    # Row 2 remaining: MALA
    axes[1][1].imshow(mala_samples_xs[-1], cmap='gray', vmin=vmin, vmax=vmax)
    axes[1][1].set_title("MALA last")
    axes[1][2].imshow(mala_mean, cmap='gray', vmin=vmin, vmax=vmax)
    axes[1][2].set_title("MALA mean")
    axes[1][3].bar(list(range(k)), mala_counts, width=1.0)
    axes[1][3].set_title("MALA last assignments")
    axes[1][3].axis("off")
    axes[1][4].bar(list(range(k)), np.mean(mala_counts_all, axis=0), width=1.0)
    axes[1][4].set_title("MALA mean assignments")
    axes[1][4].axis("off")
    ...
    classes = np.array(
        ...
        + [4 for _ in range((n_vg + 2) // 4)] 
        + [5 for _ in range((n_vg + 3) // 4)] 
        + [6 for _ in range(n_mala // 4)]
        + [7 for _ in range((n_mala + 1) // 4)]
        + [8 for _ in range((n_mala + 2) // 4)]
        + [9 for _ in range((n_mala + 3) // 4)]
    )
    imgs_all = [img_spin, xs_em_refined[-1]] + vg_samples_xs + mala_samples_xs
    imgs_all_encode, pca_ratios, pca_components, pca_mean = pca_embed_images(imgs_all, n_components=2, normalize=False)
